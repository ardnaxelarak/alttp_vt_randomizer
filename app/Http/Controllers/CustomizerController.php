...
class CustomizerController extends Controller
{
    ...
    protected function prepSeed(Request $request, bool $save = false)
    {
        ...
        $world = World::factory(1, $request->input('mode', 'standard'), array_merge([
            ...
            'bossItems' => $request->input('boss_items', 'any'),
            ...
            'doors.type_mode' => $request->input('door_type_mode', 'original'),
            'doors.trap_mode' => $request->input('trap_door_mode', 'vanilla'),
            'doors.decoupled' => $request->input('decouple_doors', 'off'),
            ...
            'overworld.whirlpoolShuffle' => $request->input('ow_whirlpool_shuffle', 'vanilla'),
        ...
        foreach ($request->input('l', []) as $location => $item) {
            ...
            if (isset($locations[$decoded_location])) {
                ...
                if (in_array($world->config('mode.weapons'), ['swordless', 'swordless_b']) && $place_item instanceof Item\Sword) {
